...
public partial class AdminPanel_MST_FinYear_MST_FinYearList : System.Web.UI.Page
{ 
    #region 11.0 Variables
	
    String FormName = "MST_FinYearList";
    ...
    private void Search(int PageNo)
    {
        ...
        SqlString FinYearName = SqlString.Null;
		...
		if (txtFinYearName.Text.Trim() != String.Empty)
			FinYearName = txtFinYearName.Text.Trim();
        ...
        DataTable dt = balMST_FinYear.SelectPage(Offset, PageRecordSize, out TotalRecords , FinYearName,  FromDate,  ToDate);
    ...
    protected void rpData_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "DeleteRecord")
        {
            try
            {
                ...
                {                    
                    if (balMST_FinYear.Delete(Convert.ToInt32(e.CommandArgument)))
                    {
	...
	if (ViewState["CurrentPage"] != null)
        ...
        MST_FinYearBAL balMST_FinYear = new MST_FinYearBAL();
        DataTable dtMST_FinYear = balMST_FinYear.SelectPage(Offset, PageRecordSize, out TotalReceivedRecord ,  FinYearName,  FromDate,  ToDate);
        if (dtMST_FinYear != null && dtMST_FinYear.Rows.Count > 0)
		{
            Session["ExportTable"] = dtMST_FinYear;
