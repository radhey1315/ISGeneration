...
public partial class AdminPanel_MST_IncomeType_MST_IncomeTypeList : System.Web.UI.Page
{ 
    #region 11.0 Variables
	
    String FormName = "MST_IncomeTypeList";
    ...
    private void Search(int PageNo)
    {
        ...
        SqlString IncomeType = SqlString.Null;
		...
		if (txtIncomeType.Text.Trim() != String.Empty)
			IncomeType = txtIncomeType.Text.Trim();
        ...
        DataTable dt = balMST_IncomeType.SelectPage(Offset, PageRecordSize, out TotalRecords , IncomeType ,  HospitalID);
    ...
    protected void rpData_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "DeleteRecord")
        {
            try
            {
                ...
                {                    
                    if (balMST_IncomeType.Delete(Convert.ToInt32(e.CommandArgument)))
                    {
	...
	if (ViewState["CurrentPage"] != null)
        ...
        MST_IncomeTypeBAL balMST_IncomeType = new MST_IncomeTypeBAL();
        DataTable dtMST_IncomeType = balMST_IncomeType.SelectPage(Offset, PageRecordSize, out TotalReceivedRecord , IncomeType , HospitalID);
        if (dtMST_IncomeType != null && dtMST_IncomeType.Rows.Count > 0)
		{
            Session["ExportTable"] = dtMST_IncomeType;
