...
{ 
    ...
    private void Search(int PageNo)
    {
        ...
        if (dt != null && dt.Rows.Count > 0)
        {
            ...
            #region IsDefault Yes Or No Class
            foreach (RepeaterItem Ri in rpData.Items)
            {
                Label lblIsDefault = (Label)Ri.FindControl("lblIsDefault");
                if (lblIsDefault.Text.Trim() == "Yes")
                {
                    lblIsDefault.CssClass = "badge badge-success";
                }
                else
                {
                    lblIsDefault.CssClass = "badge badge-danger";
                }
            }
            #endregion IsDefault Yes Or No Class
